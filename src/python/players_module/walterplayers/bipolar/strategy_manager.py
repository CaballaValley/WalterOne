...
class StrategyManager:
    ...
    def __init__(self, defensive_limit, offensive_limit):
        ...
        print('Strategy Manager created with Defensive Limit: ' + str(self._defensive_limit) + ' and Offensive Limit: ' + str(self._offensive_limit))
    ...
    def update_life_points(self, life_points):
        ...
        if not self._defenvise_mode_limit or not self._offensive_mode_limit:
            ...
            print('Mode limits initialized with Deffensive Mode Limit : ' + str(self._defenvise_mode_limit) + ' and Offensive Mode Limit: ' + str(self._offensive_mode_limit))
        

        if AdviserMode.Offensive == self._adviser_mode and life_points <= self._defenvise_mode_limit:
            self._previous_adviser_mode = self._adviser_mode
            self._adviser_mode = AdviserMode.Defensive
            return

        if AdviserMode.Defensive == self._adviser_mode and life_points >= self._offensive_mode_limit:
            self._previous_adviser_mode = self._adviser_mode
            self._adviser_mode = AdviserMode.Offensive
            return
...
'''strategyManager = StrategyManager(0.8, 0.9)
life_point = 100

while True:
    strategyManager.update_life_points(life_point)
    print('Life Point: ' + str(life_point))
    print('Defensive Mode Active : ' + str(strategyManager.get_adviser_mode()))
    print('Strategy changed : ' + str(strategyManager.is_strategy_changed()))
    if AdviserMode.Defensive == strategyManager.get_adviser_mode():
        life_point += 1
    else:
        life_point -=1
'''